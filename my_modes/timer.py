...
class ModeTimer(Mode):
    ...
    def start_timer(self, time, delay=1):
        # Tell the mode how much time it gets, if it cares.
        self.timer_update(time)
        self.timer = time
        self.delay = delay
        self.delay(name='decrement timer', event_type=None, delay=self.delay, handler=self.decrement_timer)

    def stop_timer(self):
        self.timer = 0
        self.cancel_delayed('decrement timer')

    def reset_timer(self, time, delay=1):
        # assign a new deadline and align the handler on the delay boundary
        self.stop_timer()
        self.start_timer(time, delay)
